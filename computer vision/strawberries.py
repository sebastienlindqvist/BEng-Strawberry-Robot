...
def Main():
    ...
    image_copy=cv2.resize(image_copy,(960,540))
...
def red(image):
    ...
    image=cv2.GaussianBlur(image, (31,31),0)#101
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    #lower red
    lower_red=np.array([0,150,150])#[0,120,0] hue saturation and value
    upper_red=np.array([40,255,255])#[10,255,255]
    mask1=cv2.inRange(hsv, lower_red, upper_red)
    #range for upper range
    lower_red = np.array([140,120,100])#[170,120,0]
...
def contours(image_original,image):
    IMGcontours, hierarchy = cv2.findContours(image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)#on raspberry pi it needs _,   ,_
    ...
    #print(IMGcontours[0][1])#range from 0-7
    # Draw polygonal contour + bonding rects + circles
    color=(256,256,256)
    for i in range(len(IMGcontours)):
        if(int(boundRect[i][3])>=10):#>=250
            cv2.drawContours(drawing, contours_poly, i, color)
            cv2.rectangle(drawing, (int(boundRect[i][0])-10, int(boundRect[i][1])-10),
                (int(boundRect[i][0]+boundRect[i][2])+10, int(boundRect[i][1]+boundRect[i][3])+10), color, 1)
            cv2.circle(drawing, (int(centers[i][0]), int(centers[i][1])), int(radius[i]), color, 1)
            cv2.circle(drawing, (int(centers[i][0]), int(centers[i][1])), 1, color, 1)
            drawing=cv2.putText(drawing, 'OpenCV', (int(centers[i][0]), int(centers[i][1])), font,  0.5, color, 1, cv2.LINE_AA) 
